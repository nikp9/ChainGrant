...
describe("Milestone Contract", function () {
  ...
  beforeEach(async function () {
    ...
    await measureCall(
      adminContract.connect(owner).addAdmin(admin1.address),
      "addAdmin"
    );
    await measureCall(
      adminContract.connect(owner).addAdmin(admin2.address),
      "addAdmin"
    );
    await measureCall(
      adminContract.connect(owner).addAdmin(admin3.address),
      "addAdmin"
    );
    ...
    await measureCall(
      projectContract.setMilestoneContractAddress(
        await milestoneContract.getAddress()
      ),
    ...
    await measureCall(
      validatorContract.connect(validator1).addValidator(2),
      "addValidator"
    );
    await measureCall(
      validatorContract.connect(validator2).addValidator(2),
      "addValidator"
    );
    await measureCall(
      validatorContract.connect(validator3).addValidator(2),
      "addValidator"
    );
    await measureCall(
      validatorContract.connect(validator4).addValidator(2),
      "addValidator"
    );
    await measureCall(
      validatorContract.connect(validator5).addValidator(2),
      "addValidator"
    );
    ...
    await measureCall(
      validatorContract
        .connect(owner)
        .updateValidatorStatus(validator1.address, 1),
      "updateValidatorStatus"
    );
    await measureCall(
      validatorContract
        .connect(owner)
        .updateValidatorStatus(validator2.address, 1),
      "updateValidatorStatus"
    );
    await measureCall(
      validatorContract
        .connect(owner)
        .updateValidatorStatus(validator3.address, 1),
      "updateValidatorStatus"
    );
    await measureCall(
      validatorContract
        .connect(owner)
        .updateValidatorStatus(validator4.address, 1),
      "updateValidatorStatus"
    );
    await measureCall(
      validatorContract
        .connect(owner)
        .updateValidatorStatus(validator5.address, 1),
      "updateValidatorStatus"
    );
    ...
    await measureCall(
      projectContract
        .connect(validator1)
        .updateScore(project1.address, "10110"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator2)
        .updateScore(project1.address, "11110"),
      "updateScore"
    );
    await measureCall(
      projectContract
    ...
    await measureCall(
      projectContract
        .connect(validator4)
        .updateScore(project1.address, "10111"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator5)
        .updateScore(project1.address, "11111"),
      "updateScore"
    );
    ...
    await measureCall(
      projectContract
        .connect(validator1)
        .updateScore(project2.address, "10110"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator2)
        .updateScore(project2.address, "11100"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator3)
        .updateScore(project2.address, "10100"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator4)
        .updateScore(project2.address, "10111"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator5)
        .updateScore(project2.address, "11101"),
      "updateScore"
    );
    ...
    await measureCall(
      projectContract
        .connect(validator1)
        .updateScore(project3.address, "10110"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator2)
        .updateScore(project3.address, "11110"),
      "updateScore"
    );
    await measureCall(
      projectContract
    ...
    await measureCall(
      projectContract
        .connect(validator4)
        .updateScore(project3.address, "10101"),
      "updateScore"
    );
    await measureCall(
      projectContract
        .connect(validator5)
        .updateScore(project3.address, "11101"),
      "updateScore"
    );
  ...
  describe("Deployment", function () {
    it("Should set right addresses for contract interfaces", async function () {
      const validatorAddr = await measureCall(
        projectContract.validatorContract(),
        "validatorContract"
      );
      const adminAddr = await measureCall(
        projectContract.adminContract(),
        "adminContract"
      );
      const milestoneAddr = await measureCall(
        projectContract.milestoneContractAddress(),
      ...
      await expect(
        measureCall(
          projectContract
            ...
            .setMilestoneContractAddress(ethers.ZeroAddress),
  ...
  describe("Project Selection", function () {
    ...
    it("Should set the completion status after the project is completed", async function () {
      ...
      await measureCall(
        milestoneContract
          .connect(validator1)
          .updateMilestoneStatus(project1.address, 1),
